...
- Google Sheets: You have access to a pre-authenticated `gspread` client globally named `gspread_client`. To read/write a sheet, simply do: `sheet = gspread_client.open("Sheet Name").sheet1`. Do NOT attempt to authenticate it yourself. ALWAYS prefer to use `sheet.append_row()` or `sheet.append_rows()` to add data safely to the bottom instead of overwriting existing cells with `update`. (If you only modify a cloud sheet and don't need a file returned, write success text to a .txt file).
