    ...
    try:
        ...
        while True:
            ...
            if search_start != -1 and search_end != -1 and search_end > search_start:
                ...
                # Set flag so we can trigger a rerun
                did_search = True
            ...
            if exec_start != -1 and exec_end != -1 and exec_end > exec_start:
                ...
                # Also remove the search block if it was there so the final text is clean
                if search_start != -1 and search_end != -1 and search_end > search_start:
                     clean_reply = clean_reply.replace(full_reply[search_start:search_end + len("[/WEB_SEARCH]")], "").strip()
                ...
                if search_start != -1 and search_end != -1 and search_end > search_start:
                    final_reply = full_reply.replace(full_reply[search_start:search_end + len("[/WEB_SEARCH]")], "").strip()
                else:
                    final_reply = full_reply
            
            # If we did a search AND DID NOT execute code, loop again to let the AI process the search results.
            # If we explicitly executed code, we break because the file is ready.
            if did_search and (exec_start == -1 or exec_end == -1):
