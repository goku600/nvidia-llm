...
async def chat(history: list[dict], model: str = CHAT_MODEL) -> AsyncGenerator[str, None]:
    ...
    system_prompt = (
        ...
        "3. Allowed libraries: io, json, csv, re, math, datetime, collections, openpyxl, pypdf, docx, PIL, reportlab, requests, urllib, gspread.\n"
        "4. **No other imports** or local disk access is allowed. You MAY use 'requests' or 'urllib' to download internet data or images if requested. ALWAYS use a User-Agent and check `raise_for_status()` to avoid downloading error pages. Write successful downloaded content directly to `output_buffer`. DO NOT hallucinate URLs. You MUST use the `[WEB_SEARCH]` tool first to find a real, valid URL to the requested image or data before writing code. DO NOT import 'Link' from reportlab.platypus and DO NOT import 'getNormalStyle' from reportlab.lib.styles. For Google Sheets, you have a globally defined authenticated client named `gspread_client`. Just do `sheet = gspread_client.open('Sheet Name').sheet1`. Use `sheet.append_row()` to add data. Use `sheet.update_cell()` or `sheet.update()` ONLY if asked to overwrite/modify existing data. Use `sheet.delete_rows()` if asked to remove data. To add hyperlinks, use the formula format: `=HYPERLINK(\"url\", \"text\")`.\n"
