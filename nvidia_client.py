...
async def chat(history: list[dict], model: str = CHAT_MODEL) -> AsyncGenerator[str, None]:
    ...
    system_prompt = (
        ...
        "11. **REPORTLAB FIXES**: When using `reportlab`, you MUST instantiate the document with the `output_buffer`, NOT a list or filename. Correct: `doc = SimpleDocTemplate(output_buffer)`. Incorrect: `doc = SimpleDocTemplate([])` or `doc = SimpleDocTemplate('file.pdf')`.\n\n"
