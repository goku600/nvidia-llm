...
async def chat(history: list[dict], model: str = CHAT_MODEL) -> AsyncGenerator[str, None]:
    ...
    system_prompt = (
        ...
        "3. Allowed libraries: io, json, csv, re, math, datetime, collections, openpyxl, pypdf, docx, PIL, reportlab, requests, urllib, gspread, pandas, numpy, bs4.\n"
        ...
        "5. **GOOGLE SHEETS (DATABASE) ACCESS**: For Google Sheets (often called 'database' or 'docs database'), you have a globally defined authenticated client named exactly `gspread_client`. Just do `sheet = gspread_client.open('Sheet Name').sheet1`. **CRITICAL:** DO NOT write your own authentication code! DO NOT use `oauth2client` and NEVER try to load `client_secret.json`. Just use the pre-existing `gspread_client`. Use `sheet.append_row()` to add data. Use `sheet.update()`, `sheet.delete_rows()` ONLY if asked to overwrite or remove. If asked to 'read' or 'show' database data, you MUST pull all records (`data = sheet.get_all_records()`), convert to a pandas DataFrame (`df = pandas.DataFrame(data)`), and save it to the `output_buffer` as a CSV (`df.to_csv(output_buffer, index=False)`) or string TXT file so the user can download it. To add hyperlinks locally, use the formula format: `=HYPERLINK(\"url\", \"text\")`.\n"
        "6. If a user previously uploaded a document, its raw bytes are available in the variable `input_bytes` (type: bytes).\n"
        "7. Write your output to the pre-existing variable `output_buffer` (type: io.BytesIO). DO NOT use `open()` to save files, as it is strictly blocked. If loading an internet image into PIL, you MUST verify it succeeds and wrap the bytes: `Image.open(io.BytesIO(r.content))`.\n"
